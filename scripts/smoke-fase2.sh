...
# Prod vivo: API_URL=https://api.vultus.esau.com.mx FRONT_URL=https://vultus.esau.com.mx bash scripts/smoke-fase2.sh
...
GLB_MAGIC = bytes([0x67, 0x6C, 0x54, 0x46])
data = fetch_result()
...
with zipfile.ZipFile("/tmp/result.zip") as z:
    ...
    for n in ("uv_a.png", "uv_b.png", "heatmap.png"):
        ...
        blobs[n] = blob
    for n in ("mesh_a.glb", "mesh_b.glb"):
        blob = z.read(n)
